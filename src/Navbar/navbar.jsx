import React from 'react';
import { NavLink, Link } from 'react-router-dom';
import { Menu, X, Zap, RefreshCcw } from 'lucide-react';
import { useNavbar } from './useNavbar';
import { navItems } from './navdata';
import { useLang } from '../i18n/LanguageContext';

// Map navItem label → translation key
const NAV_KEY_MAP = {
  Home: "nav.home", "News Feed": "nav.newsFeed", Global: "nav.global",
  National: "nav.national", State: "nav.state", Business: "nav.business",
  Crime: "nav.crime", Entertainment: "nav.entertainment", Sports: "nav.sports",
  Health: "nav.health", Politics: "nav.politics", Travel: "nav.travel",
  Admin: "nav.admin", "Live TV": "nav.liveTV", Trending: "nav.trending",
  "ID Card": "nav.idCard", Login: "nav.login",
};

const Navbar = () => {
  const { isExpanded, isMobile, toggleNavbar } = useNavbar();
  const { t } = useLang();

  if (isMobile) return null;

  return (
    <aside className={`
      hidden md:flex flex-col
      fixed inset-y-0 left-0 z-50 h-screen bg-slate-900 text-slate-300
      transition-all duration-300 ease-in-out shadow-2xl border-r border-slate-800
      ${isExpanded ? 'w-64' : 'w-20'}
    `}>
      {/* Logo & Toggler */}
      <div className="flex items-center justify-between p-6 h-[80px] border-b border-slate-800 bg-slate-900/50 backdrop-blur-md mt-[110px]">
        <div className="flex items-center gap-2">
          <Zap className="text-[#1DB954] fill-[#1DB954]" size={20} />
          {isExpanded && (
            <span className="font-black text-white text-lg uppercase italic tracking-tighter">
              AP13 News
            </span>
          )}
        </div>
        <button
          onClick={toggleNavbar}
          className="text-slate-500 hover:text-white ml-auto transition-colors p-2 hover:bg-slate-800 rounded-lg"
        >
          {isExpanded ? <X size={20} /> : <Menu size={20} />}
        </button>
      </div>

      {/* Nav items */}
      <nav className="mt-4 px-3 space-y-1 flex-1 overflow-y-auto custom-scrollbar">
        {navItems.map((item) => (
          <NavLink
            key={item.label}
            to={item.path}
            className={({ isActive }) => `
              relative flex items-center gap-4 px-3 py-3 rounded-xl transition-all group
              ${isActive
                ? 'bg-[#1DB954]/10 text-[#1DB954]'
                : 'hover:bg-slate-800 hover:text-white'}
            `}
          >
            {({ isActive }) => (
              <>
                {isActive && (
                  <div className="absolute left-0 w-1.5 h-6 bg-[#1DB954] rounded-r-full shadow-[4px_0_12px_rgba(29,185,84,0.5)]" />
                )}
                <item.icon
                  size={22}
                  className={`min-w-[22px] transition-transform duration-300 ${isActive ? 'scale-110 text-[#1DB954]' : 'group-hover:scale-110'}`}
                />
                {isExpanded && (
                  <span className={`font-bold text-sm tracking-tight transition-colors ${isActive ? 'text-white' : 'group-hover:text-white'}`}>
                    {t(NAV_KEY_MAP[item.label] || `nav.${item.label.toLowerCase()}`)}
                  </span>
                )}
              </>
            )}
          </NavLink>
        ))}
      </nav>

      {/* System update link */}
      <div className="mt-auto p-4 border-t border-slate-800">
        <Link
          to="/update-data"
          className="flex items-center gap-4 px-3 py-3 rounded-xl text-slate-500 hover:bg-slate-800 hover:text-[#1DB954] transition-all group"
        >
          <RefreshCcw size={22} className="min-w-[22px] group-hover:rotate-180 transition-transform duration-500" />
          {isExpanded && <span className="font-bold text-sm">System Update</span>}
        </Link>
      </div>
    </aside>
  );
};

export default Navbar;