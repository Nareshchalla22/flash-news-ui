...
const FIELD_META = {
  ...
  imageUrl:         { label: "Image",           type: "image"    }, // ← handled specially
...
// ─── IMAGE UPLOAD to S3 via your backend ──────────────────────────────────────
...
// Convert file to base64 (fallback if S3 not configured)
...
function ImageUploadField({ value, onChange }) {
  ...
  const processFile = async (file) => {
    ...
    try {
      const localPreview = await toBase64(file);
      onChange(localPreview); // show preview right away
    } catch (_) {}
    ...
    try {
      ...
      // S3 failed — keep base64 preview as fallback
      clearInterval(tick);
      console.warn("S3 upload failed, using base64 fallback:", uploadErr.message);
      // base64 is already set above — just clear progress
      setProgress(0);
      setError("⚠ Saved as preview only — S3 not configured. Image will show but won't persist after refresh.");
    } finally {
      setUploading(false);
  ...
  // Check if current value is a real URL (not base64)
  ...
  return (
    <div>
      ...
      <div className="flex gap-1 mb-2 p-0.5 bg-slate-950 rounded-lg w-fit">
        <button
          ...
          className={`px-3 py-1 rounded-md text-xs font-bold uppercase tracking-wider transition-all cursor-pointer
            ${uploadMode === "upload" ? "bg-slate-700 text-white" : "text-slate-500 hover:text-slate-300"}`}
        >
          📁 Upload File
        </button>
        <button
          ...
          className={`px-3 py-1 rounded-md text-xs font-bold uppercase tracking-wider transition-all cursor-pointer
            ${uploadMode === "url" ? "bg-slate-700 text-white" : "text-slate-500 hover:text-slate-300"}`}
        >
          🔗 Paste URL
        </button>
      </div>
          ...
          {hasImage && (
            <div className="relative w-full h-32 rounded-lg overflow-hidden border border-slate-700">
              ...
              <button
                ...
                className="absolute top-2 right-2 w-6 h-6 rounded-full bg-red-600 text-white text-xs flex items-center justify-center cursor-pointer hover:bg-red-700"
              >✕</button>
            </div>
          )}
        </div>
        ...
        <div>
          ...
          {hasImage ? (
            ...
            <div className="relative w-full rounded-xl overflow-hidden border border-slate-700 bg-slate-950">
              ...
              <div className="absolute inset-0 bg-black/0 hover:bg-black/50 transition-all flex items-center justify-center gap-3 opacity-0 hover:opacity-100">
                <button
                  ...
                  className="px-3 py-1.5 bg-white/90 text-slate-900 rounded-lg text-xs font-bold cursor-pointer hover:bg-white transition-colors"
                >
                  🔄 Change
                </button>
                <button
                  type="button"
                  onClick={handleRemove}
                  className="px-3 py-1.5 bg-red-600/90 text-white rounded-lg text-xs font-bold cursor-pointer hover:bg-red-600 transition-colors"
                >
                  🗑 Remove
                </button>
              </div>
              ...
              {/* S3 badge */}
              {isS3Url && (
                <div className="absolute bottom-2 left-2 px-2 py-0.5 bg-green-600/90 rounded text-[9px] font-bold text-white uppercase tracking-wider">
                  ✓ S3 Uploaded
                </div>
              )}
              {isBase64 && (
                <div className="absolute bottom-2 left-2 px-2 py-0.5 bg-yellow-600/90 rounded text-[9px] font-bold text-white uppercase tracking-wider">
                  ⚠ Local Preview
                </div>
              )}
            </div>
            ...
            <div
              ...
              className={`
                relative w-full h-44 rounded-xl border-2 border-dashed
                flex flex-col items-center justify-center gap-3
                transition-all cursor-pointer
                ${dragOver
                  ? "border-blue-400 bg-blue-500/10 scale-[1.01]"
                  : "border-slate-700 hover:border-slate-500 bg-slate-950 hover:bg-slate-900/50"}
            ...
            >
              {uploading ? (
                <div className="flex flex-col items-center gap-3">
                  {/* Spinner */}
                  <div className="w-10 h-10 rounded-full border-3 border-slate-700 border-t-blue-400 animate-spin" style={{ borderWidth: 3 }} />
                ...
                <>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-1 bg-slate-800 rounded text-[10px] font-bold text-slate-500 uppercase">JPG</span>
                    <span className="px-2 py-1 bg-slate-800 rounded text-[10px] font-bold text-slate-500 uppercase">PNG</span>
                    <span className="px-2 py-1 bg-slate-800 rounded text-[10px] font-bold text-slate-500 uppercase">WebP</span>
                    <span className="px-2 py-1 bg-slate-800 rounded text-[10px] font-bold text-slate-500 uppercase">GIF</span>
                  </div>
                </>
              )}
            </div>
      ...
      {/* Error message */}
      {error && (
        <p className="mt-2 text-xs text-yellow-400 font-semibold flex items-start gap-1.5">
  ...
  return (
    <div className="flex flex-col gap-5">
      {fields.map(field => {
        ...
        if (!meta) return null;
        const isReporterField = field === "reporterName";
        const isImageField    = meta.type === "image";
        ...
        return (
          <div key={field}>
            <label className="flex items-center gap-2 text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">
              {meta.label}
              {isReporterField && (
                <span className="text-[9px] bg-green-500/20 text-green-400 border border-green-500/30 px-1.5 py-0.5 rounded font-bold normal-case tracking-normal">
                  Auto-filled
                </span>
              )}
              {isImageField && (
                <span className="text-[9px] bg-blue-500/20 text-blue-400 border border-blue-500/30 px-1.5 py-0.5 rounded font-bold normal-case tracking-normal">
                  Local Upload
                </span>
              )}
            </label>
            ...
            {isImageField ? (
              ...
              <input
                ...
                value={form[field] || ""}
                onChange={e => onChange(field, e.target.value)}
                placeholder={isReporterField ? autoReporterName || "" : ""}
                className={`w-full px-3 py-2.5 rounded-lg bg-slate-950 border text-slate-200 text-sm outline-none focus:border-slate-600 transition-colors
                  ${isReporterField ? "border-green-500/30 focus:border-green-500/60" : "border-slate-800"}`}
              />
            )}
          </div>
        );
...
function CreateModal({ category, catKey, onClose, onSuccess, reporterName }) {
  ...
  const initialForm = Object.fromEntries(
    fields.map(f => [f, f === "reporterName" ? reporterName || "" : ""])
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      onSuccess("New entry published successfully!");
      onClose();
    } catch (err) {
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-lg max-h-[92vh] overflow-hidden flex flex-col shadow-2xl">
        ...
        <div className="p-5 overflow-y-auto flex-1">
          ...
          <div className="flex gap-3 mt-5">
            <button onClick={onClose}
              className="flex-1 py-3 rounded-xl bg-slate-800 border border-slate-700 text-slate-400 hover:text-slate-200 font-bold text-sm cursor-pointer transition-colors">
              Cancel
            </button>
            <button onClick={handleSubmit} disabled={loading}
              className={`flex-[2] py-3 rounded-xl text-white font-black italic text-sm uppercase tracking-wider cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2 transition-colors ${colors.btn}`}>
              {loading
...
function EditModal({ record, category, catKey, onClose, onSuccess, reporterName }) {
  ...
  const handleUpdate = async () => {
    setLoading(true); setError("");
    try {
      await apiFetch(`${catKey}/${record.id}`, "PUT", form);
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-lg max-h-[92vh] overflow-hidden flex flex-col shadow-2xl">
        ...
        <div className={`px-5 py-4 border-b border-slate-800 flex items-center justify-between flex-shrink-0 ${colors.bg}`}>
          <div>
            <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">
              Edit Record <span className="text-slate-700">#{record.id}</span>
            </p>
            <h3 className={`text-lg font-black italic ${colors.text}`}>
              Update <span className="text-slate-100">{category}</span>
            </h3>
          </div>
          <button onClick={onClose}
            className="w-8 h-8 rounded-lg bg-slate-800 border border-slate-700 text-slate-400 hover:text-white flex items-center justify-center cursor-pointer transition-colors">
        ...
        <div className="px-5 py-2 bg-slate-950 border-b border-slate-800 flex-shrink-0">
          <p className="text-xs text-slate-600 font-semibold">
            Editing: <span className="text-slate-400 font-bold">
              {record.title || record.matchTitle || record.movieTitle ||
               record.companyName || record.gadgetHead || `Record #${record.id}`}
            </span>
          </p>
        </div>
        ...
        <div className="p-5 overflow-y-auto flex-1">
          ...
          <div className="flex gap-3 mt-5">
            ...
            <button onClick={handleUpdate} disabled={loading}
              className="flex-[2] py-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-black italic text-sm uppercase tracking-wider cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2 transition-colors">
              {loading
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-sm w-full shadow-2xl text-center">
        ...
        <div className="flex gap-3">
          <button onClick={onCancel}
            className="flex-1 py-2.5 rounded-xl bg-slate-800 border border-slate-700 text-slate-400 hover:text-slate-200 font-bold text-sm cursor-pointer transition-colors">
            Cancel
          </button>
...
function UsersTab() {
  ...
  const handleToggle = async (id, enabled) => {
    try {
      await apiFetch(`auth/users/${id}/toggle`, "PATCH");
      showToast(enabled ? "User disabled" : "User enabled");
      load();
            ...
            <tbody>
              {loading ? (
                ...
                users.map((u, i) => {
                  ...
                  return (
                    <tr key={u.id}
                      className={`border-b border-slate-800/50 hover:bg-white/[0.02] ${i % 2 === 0 ? "bg-slate-950/50" : ""}`}>
                      <td className="px-4 py-3">
                        <span className="text-xs font-bold text-slate-600 bg-slate-800 px-2 py-1 rounded-lg">#{u.id}</span>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-xs font-bold px-2 py-1 rounded-lg border uppercase tracking-wider ${roleColor}`}>{u.role}</span>
                      </td>
                      <td className="px-4 py-3"><span className="text-xs text-slate-500">{u.planName || "—"}</span></td>
                      <td className="px-4 py-3">
                        <span className={`text-xs font-bold px-2 py-1 rounded-full ${u.enabled ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}>
...
export default function AdminDashboard() {
  ...
  const getRecordName  = (r)   =>
    r.title || r.matchTitle || r.movieTitle || r.companyName || r.gadgetHead || `#${r.id}`;
  ...
  const filtered = records.filter(r =>
    !search || catConfig.fields.some(f =>
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200">
        ...
        `}>
          ...
          <div className="mt-4 pt-4 border-t border-slate-800">
            ...
            {isAdmin && (
              <>
                <a href="/admin/applications"
                  className="w-full flex items-center px-3 py-2 rounded-xl mb-1 text-xs font-bold uppercase tracking-wider cursor-pointer text-yellow-400 bg-yellow-500/10 border border-yellow-500/20 hover:bg-yellow-500/20">
                  📋 Applications
                </a>
                <a href="/ticker-control"
                  className="w-full flex items-center px-3 py-2 rounded-xl mb-1 text-xs font-bold uppercase tracking-wider cursor-pointer text-blue-400 bg-blue-500/10 border border-blue-500/20 hover:bg-blue-500/20">
                  ⚡ Ticker Control
                </a>
              </>
            )}
              ...
              <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-6">
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-semibold pl-3">
                    {records.length} record{records.length !== 1 ? "s" : ""} · {filtered.length} shown
                    {reporterName && (
                      <span className="ml-2 text-green-500">
                        · Publishing as <strong>{reporterName}</strong>
                      </span>
                    )}
                  </p>
                </div>
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
                              ...
                              <td key={f} className="px-4 py-3">
                                {f === "imageUrl" ? (
                                  <div className="w-12 h-9 rounded-lg bg-slate-800 border border-slate-700 overflow-hidden flex items-center justify-center text-slate-600 text-xs">
                                    {r[f]
                                      ? <img src={r[f]} alt="" className="w-full h-full object-cover"
                ...
                {!loading && filtered.length > 0 && (
                  <div className="px-4 py-3 border-t border-slate-800 flex items-center justify-between">
                    ...
                    {search && (
                      <button onClick={() => setSearch("")} className="text-xs text-slate-500 hover:text-slate-300 font-bold cursor-pointer">
                        Clear filter ✕
                      </button>
                    )}
                  </div>
      ...
      {showCreate && (
        <CreateModal
          category={catConfig.label} catKey={activeCategory}
      ...
      {editRecord && (
        <EditModal
          record={editRecord} category={catConfig.label} catKey={activeCategory}
